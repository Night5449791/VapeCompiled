...
run(function()
	...
	local shakeSpeed = 8
	local minFollowY, maxFollowY = -50, 999999
	...
	local function startFling(targetPlayer)
		...
		CarFling:Clean(runService.Heartbeat:Connect(function(deltaTime)
			...
			if direction then
				local speed = math.max(horizontalVelocity.Magnitude, 20)
				local leadTime = math.max(0, speed * getPingLead(targetPlayer, speed) * predictionMultiplier - leadPullback)
				...
				KickExploit:Clean(runService.Heartbeat:Connect(function(dt)
					...
					if AutoRejoin.Enabled then
						if countTimer < now then
							countTimer = now + 1
							plrCount = #teams.Guards:GetPlayers() + #teams.Inmates:GetPlayers() + #teams.Criminals:GetPlayers()
						end
